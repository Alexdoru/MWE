...
public class ArrowHitLeapHitEvent {
	...
	public static boolean processMessage(String rawMessage) {
		...
		Matcher hitMatcher0 = Pattern.compile("^\\w+ is on 0 HP!").matcher(rawMessage);
		Matcher hitMatcher1 = Pattern.compile("^\\w+ is on (\\d+) HP!").matcher(rawMessage);
		Matcher hitMatcher2 = Pattern.compile("^\\w+ is on (\\d+\\.\\d+) HP!").matcher(rawMessage);		
		Matcher hitMatcher3 = Pattern.compile("^\\w+ is on (\\d+) HP, pinned by (\\d+) arrows.*").matcher(rawMessage);// renegade
		Matcher hitMatcher4 = Pattern.compile("^\\w+ is on (\\d+\\.\\d+) HP, pinned by (\\d+) arrows.*").matcher(rawMessage);// renegade
		Matcher HitLeapMatcher = Pattern.compile("^You took (\\d+\\.\\d+) recoil damage after traveling \\d+.\\d+ blocks!").matcher(rawMessage);//leap
		Matcher DirectHitLeapMatcher = Pattern.compile("^You landed a direct hit against \\w+, taking (\\d+\\.\\d+) recoil damage after traveling \\d+\\.\\d+ blocks!").matcher(rawMessage);//leap
