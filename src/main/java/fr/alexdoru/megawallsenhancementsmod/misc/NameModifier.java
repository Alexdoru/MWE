...
public class NameModifier {
	...
	public static IChatComponent getTransformedDisplayName(GameProfile gameProfile, IChatComponent displayName) {
		...
		// ajouter un hook la dedans pour update les pseudos
		// 0 : create a team
		// 1 : remove a team
		...
		if(squadname != null) { // squad
			...
			} else if(wdr.hacks.contains("bhop")) {
		...
		if(displayName != null) {
			...
			if(needtochange) {
				return (IChatComponent) new ChatComponentText(extraprefix).appendSibling(new ChatComponentText( (issquadmate ? formattedname.replace(username, squadname).replace("\u00a7k","") : formattedname.replace("\u00a7k",""))));
			} else {
		...
		if(team != null) { 
			...
			if(needtochange) {
				return (IChatComponent) new ChatComponentText(extraprefix + teamprefix.replace("\u00a7k","").replace("O","") + (issquadmate ? squadname : username) + team.getColorSuffix());
			}
		}
