...
public class CommandStalkList extends CommandBase {
	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		...
		if(args.length >= 1 && (args[0].equals("list") | args[0].equals("l"))) {
			...
			if(args.length >= 2) {
				...
				ChatUtil.addChatMessage((IChatComponent)new ChatComponentText(ChatUtil.getTagMW() + EnumChatFormatting.GREEN + "Making player list for : " + listkey + "..."));
				...
				ChatUtil.addChatMessage((IChatComponent)new ChatComponentText(EnumChatFormatting.RED + "Usage : " + getCommandUsage(sender) + " list <listname> <page(optional)>"));
				return;
			}
			...
			if(args.length != 3) {
				ChatUtil.addChatMessage((IChatComponent)new ChatComponentText(EnumChatFormatting.RED + "Usage : " + getCommandUsage(sender) + " move <uuid> <newlistname>"));
				return;
			} else {
		...
		ChatUtil.addChatMessage((IChatComponent)new ChatComponentText(ChatUtil.getTagMW() + EnumChatFormatting.GREEN + "Using : \""+ keyword + "\", Stalking " + i + " players..."));
	}
	...
	private void movePlayerTo(String uuid, String newlist) {
		...
		if(wdr == null) {
			ChatUtil.addChatMessage((IChatComponent)new ChatComponentText(EnumChatFormatting.RED + "This uuid is not report list"));
			return;
		} else {			
			String newlistname = newlist.contains("stalk") ? newlist : "stalk" + newlist;			
			ArrayList<String> newHacksArray = new ArrayList<String>();					
			...
			ChatUtil.addChatMessage((IChatComponent)new ChatComponentText(ChatUtil.getTagMW() + EnumChatFormatting.GREEN + "Successfully moved this player to : " + EnumChatFormatting.GOLD + newlistname));
		}
